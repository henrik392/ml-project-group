...
def plot_training_curves(df: pd.DataFrame, output_dir: Path, run_name: str):
    ...
    ax = axes[0, 0]
    ax.plot(df['epoch'], df['train/box_loss'], 'b-', label='Train Box Loss', linewidth=2)
    ax.plot(df['epoch'], df['val/box_loss'], 'b--', label='Val Box Loss', linewidth=2, alpha=0.7)
    ax.plot(df['epoch'], df['train/cls_loss'], 'r-', label='Train Cls Loss', linewidth=2)
    ax.plot(df['epoch'], df['val/cls_loss'], 'r--', label='Val Cls Loss', linewidth=2, alpha=0.7)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ...
    ax = axes[0, 1]
    ax.plot(df['epoch'], df['metrics/precision(B)'], 'g-', label='Precision', linewidth=2, marker='o', markersize=3)
    ax.plot(df['epoch'], df['metrics/recall(B)'], 'orange', label='Recall', linewidth=2, marker='s', markersize=3)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Score')
    ...
    # 3. mAP scores
    ax = axes[1, 0]
    ax.plot(df['epoch'], df['metrics/mAP50(B)'], 'purple', label='mAP@0.5', linewidth=2, marker='D', markersize=3)
    ax.plot(df['epoch'], df['metrics/mAP50-95(B)'], 'brown', label='mAP@0.5:0.95', linewidth=2, marker='^', markersize=3)
    ax.set_xlabel('Epoch')
...
def plot_metrics_summary(df: pd.DataFrame, output_dir: Path, run_name: str):
    ...
    bars = ax.bar(final_metrics.keys(), final_metrics.values(), color=colors, alpha=0.8, edgecolor='black')
    ...
    ax.set_title(f'Final Validation Metrics - {run_name}', fontsize=14, fontweight='bold')
...
def plot_loss_components(df: pd.DataFrame, output_dir: Path, run_name: str):
    ...
    for ax, (train_col, val_col, title) in zip(axes, loss_types):
        ...
        ax.plot(df['epoch'], df[val_col], 'r--', label='Validation', linewidth=2, alpha=0.7)
        ax.set_xlabel('Epoch')
        ax.set_ylabel('Loss')
        ax.set_title(title)
        ax.legend()
...
def generate_statistics_table(df: pd.DataFrame) -> dict:
    ...
    stats = {
        ...
        'loss_improvement': {
            'box_loss': ((df['train/box_loss'].iloc[0] - df['train/box_loss'].iloc[-1]) /
                        df['train/box_loss'].iloc[0] * 100),
            'cls_loss': ((df['train/cls_loss'].iloc[0] - df['train/cls_loss'].iloc[-1]) /
                        df['train/cls_loss'].iloc[0] * 100),
            'dfl_loss': ((df['train/dfl_loss'].iloc[0] - df['train/dfl_loss'].iloc[-1]) /
                        df['train/dfl_loss'].iloc[0] * 100)
        }
    }
